...
for tt1=1:12%length(aa1)
    nn = nn + 1 ;
    filename1=aa1(tt1,1).name;
    SW_S_N=ncread(filename1,'FSNS'); % Downwelling solar flux at surface (W/m2)
    SW_S_d_all(:,:,nn) = SW_S_N;      
end
...
[Z_adjust(:,:,1),refvec_bias(:,:,1)] = geoloc2grid(squeeze(lat_T31(:,:,1)),squeeze(lon_T31(:,:,1)),squeeze(SST_T31(:,:,1)'), cellsize);   
 ...
 lonlim = [1 360];
        ...
        fig_name = strcat('SW_S_NET_bias_annual_Chey_ctrl_FILE_CAM_coef_NEW_CERES');%,num2str(tt));
        fig_dum = figure(100);
...
[Z_adjust(:,:,1),refvec_bias(:,:,1)] = geoloc2grid(squeeze(lat_T31(:,:,1)),squeeze(lon_T31(:,:,1)),squeeze(SST_T31(:,:,1)'), cellsize);   
 ...
 lonlim = [1 360];
        ...
        fig_name = strcat('SW_S_NET_bias_diff_annual_Chey_ctrl_CAM_coef_NEW_CERES');%,num2str(tt));
        fig_dum = figure(144);
      set(fig_dum, 'name', fig_name,'numbertitle','on');
      set(fig_dum,'units','inches','position',[0.3,0.3,12,9]);%,'PaperOrientation','landscape');
      set(fig_dum,'paperpositionmode','auto');
...
%%%%%
dbstop   
 nccreate('Bias_S_N_daily_CAM_coef_Chey_ctrl.nc','Bias_S_N_daily','Dimensions',{'lon',96,'lat',48,'day',365},'Format','classic');
 nccreate('Bias_S_N_daily_CAM_coef_Chey_ctrl.nc','lat','Dimensions',{'lon',96,'lat',48},'Format','classic');
 nccreate('Bias_S_N_daily_CAM_coef_Chey_ctrl.nc','lon','Dimensions',{'lon',96,'lat',48},'Format','classic'); 
 ncwrite('Bias_S_N_daily_CAM_coef_Chey_ctrl.nc','Bias_S_N_daily',Bias_S_d_2_dail_mea);
 ncwrite('Bias_S_N_daily_CAM_coef_Chey_ctrl.nc','lat',lat_msh');
 ncwrite('Bias_S_N_daily_CAM_coef_Chey_ctrl.nc','lon',lon_msh');
...
[Z_adjust(:,:,1),refvec_bias(:,:,1)] = geoloc2grid(squeeze(lat_T31(:,:,1)),squeeze(lon_T31(:,:,1)),squeeze(SST_T31(:,:,1)'), cellsize);   
 ...
 lonlim = [1 360];
        ...
        fig_name = strcat('SW_S_NET_bias_annual_FILE_CAM_coef_NEW_CERES',num2str(tt));
...
AA=geoshow(squeeze(Z_adjust(:,:,1)), squeeze(refvec_bias(:,:,1)), 'DisplayType', 'texturemap','EdgeColor','flat');
    ...
    setm(gca,'FEdgeColor',[1 1 1]);
  m_coast('linewidth',2,'color','black');
    Title_1 = strcat('Bias, CESM(ctrl) / CERES, Net SW Flux at Surface, Annual Mean, CAM grid, ',month(tt));
    title(Title_1,'fontsize',23,'fontweight','bold');
    cc = colorbar('peer',gca);
    set(gca,'Fontsize',20)
    m_grid('linewi',2,'linest','none','tickdir','in','fontsize',20);
...
Test_SW = ncread('Bias_S_N_daily_CAM_coef_Chey_ctrl.nc','Bias_S_N_daily') ;
Test_lat = ncread('Bias_S_N_daily_CAM_coef_Chey_ctrl.nc','lat') ;
Test_lon = ncread('Bias_S_N_daily_CAM_coef_Chey_ctrl.nc','lon') ;
...
[Z_adjust(:,:,1),refvec_bias(:,:,1)] = geoloc2grid(squeeze(lat_T31(:,:,1)),squeeze(lon_T31(:,:,1)),squeeze(SST_T31(:,:,1)), cellsize);   
 ...
 lonlim = [1 360];
        ...
        fig_name = strcat('SW_Surf_NET_bias_annual_fromFILE_CAM_CAMgrid_diff_prescr_SST_NEW_NEW_CERES');%,num2str(tt));
        fig_dum = figure(100);
      set(fig_dum, 'name', fig_name,'numbertitle','on');
      set(fig_dum,'units','inches','position',[0.3,0.3,12,9]);%,'PaperOrientation','landscape');
      set(fig_dum,'paperpositionmode','auto');
