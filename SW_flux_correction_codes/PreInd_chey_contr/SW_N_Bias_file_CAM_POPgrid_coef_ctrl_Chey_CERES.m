...
for tt1=1:12%length(aa1)
    nn = nn + 1 ;
    filename1=aa1(tt1,1).name;
    SW_S_d=ncread(filename1,'FSNS'); % solar flux at surface (W/m2)
...
for tt = 1:12
    ...
    tmp2 = griddata(lon_fn_msh,lat_fn_msh,tmp,lon_msh_p,lat_msh_p,'natural');
    SW_S_d_mo_mean_intrp_CAMgrid(:,:,tt) = tmp2' ; 
    tmp = double(SW_S_d_all(:,:,tt)') ;
    tmp2 = griddata(lon_msh,lat_msh,tmp,lon_msh_p,lat_msh_p,'natural');
    SW_S_d_all_intrp_CAMgrid(:,:,tt) = tmp2' ;     
end

%%%%%% initial method %%%%%%%
Bias_S_d = SW_S_d_all_intrp_CAMgrid ./ SW_S_d_mo_mean_intrp_CAMgrid ;
...
%%%%% new method %%%%%%%%
SW_S_d_all_2 = SW_S_d_all_intrp_CAMgrid ;
SW_S_d_mo_mean_intrp_CAMgrid_2 = SW_S_d_mo_mean_intrp_CAMgrid ;
for k = 1:size(SW_S_d_all_2,3)
    for i = 1:size(SW_S_d_all_2,1)
        for j = 1:size(SW_S_d_all_2,2)
            if (SW_S_d_all_intrp_CAMgrid(i,j,k) <= 2)
...
for tt=1:12%length(aa1)
    ...
    TEMP=ncread(filename,'TEMP'); % solar flux at surface (W/m2)
    SST_all(:,:,tt) = TEMP(:,:,1);
end
SST_mean = nanmean(SST_all,3)';
    ...
    TAREA2(II) = NaN;
for tt = 1:365
    temp = Bias_S_d_2_dail_mea(:,:,tt) ;
    temp(II) = NaN;
    Bias_S_d_2_dail_mea(:,:,tt) = temp ;
%    temp2 = SW_S_d_mo_mean_intrp_CAMgrid(:,:,tt) ;
%    temp2(II) = NaN;
%    SW_S_d_mo_mean_intrp_CAMgrid(:,:,tt) = temp2;
end
for tt = 1:12
    temp = SW_S_d_all_intrp_CAMgrid(:,:,tt) ;
    temp(II) = NaN;
    SW_S_d_all_intrp_CAMgrid(:,:,tt) = temp ;
    temp2 = SW_S_d_mo_mean_intrp_CAMgrid(:,:,tt) ;
    temp2(II) = NaN;
    SW_S_d_mo_mean_intrp_CAMgrid(:,:,tt) = temp2;
end
...
SW_S_d_prescr_SST_CESM_month = nanmean(SW_S_d_all_intrp_CAMgrid,3) ;
SW_S_d_prescr_SST_CESM_month_mean = nansum(nansum(TAREA2' .* SW_S_d_prescr_SST_CESM_month,1),2) ./ nansum(nansum(TAREA2',1),2)

Bias_SW_S_d_mean = nanmean(SW_S_d_all_intrp_CAMgrid - SW_S_d_mo_mean_intrp_CAMgrid,3) ;
Bias_SW_S_d_mean_ave = nansum(nansum(TAREA2' .* Bias_SW_S_d_mean,1),2) ./ nansum(nansum(TAREA2',1),2)

%%%%%
  cellsize = 0.5; 
SST_T31(:,:,1)  = double(nanmean(SW_S_d_all_intrp_CAMgrid - SW_S_d_mo_mean_intrp_CAMgrid,3)) ;
lat_T31(:,:,1)  = lat_msh_p ;
lon_T31(:,:,1)  = lon_msh_p ; 
[Z_adjust(:,:,1),refvec_bias(:,:,1)] = geoloc2grid(squeeze(lat_T31(:,:,1)),squeeze(lon_T31(:,:,1)),squeeze(SST_T31(:,:,1)'), cellsize);   
 latlim = [-75 75];
 lonlim = [1 360];
        ...
        fig_name = strcat('SW_S_N_bias_diff_annual_Chey_ctrl_CAM_coef_NEW_CERES');%,num2str(tt));
        fig_dum = figure(144);
      set(fig_dum, 'name', fig_name,'numbertitle','on');
      set(fig_dum,'units','inches','position',[0.3,0.3,12,9]);%,'PaperOrientation','landscape');
      set(fig_dum,'paperpositionmode','auto');
...
AA=geoshow(squeeze(Z_adjust(:,:,1)), squeeze(refvec_bias(:,:,1)), 'DisplayType', 'texturemap','EdgeColor','flat');
    ...
    setm(gca,'FEdgeColor',[1 1 1]);
  m_coast('linewidth',2,'color','black');
    Title_1 = 'Bias, CESM(ctrl) - CERES, Net SW Flux at Surface, Annual Mean, CAM grid';
    title(Title_1,'fontsize',23,'fontweight','bold');
    cc = colorbar('peer',gca);
    set(gca,'Fontsize',20)
    m_grid('linewi',2,'linest','none','tickdir','in','fontsize',20);
...
%%%%%
dbstop
clear lat_msh lon_msh
lat_msh = lat_msh_p ; lon_msh = lon_msh_p ; Bias_S_d_2_dail_mea = permute(Bias_S_d_2_dail_mea,[2 1 3]) ;
 nccreate('Bias_S_N_CAM_POPgrid_coef_Chey_ctrl.nc','Bias_S_N_daily','Dimensions',{'lon',100,'lat',116,'day',365},'Format','classic');
 nccreate('Bias_S_N_CAM_POPgrid_coef_Chey_ctrl.nc','lat','Dimensions',{'lon',100,'lat',116},'Format','classic');
 nccreate('Bias_S_N_CAM_POPgrid_coef_Chey_ctrl.nc','lon','Dimensions',{'lon',100,'lat',116},'Format','classic'); 
 ncwrite('Bias_S_N_CAM_POPgrid_coef_Chey_ctrl.nc','Bias_S_N_daily',Bias_S_d_2_dail_mea);
 ncwrite('Bias_S_N_CAM_POPgrid_coef_Chey_ctrl.nc','lat',lat_msh);
 ncwrite('Bias_S_N_CAM_POPgrid_coef_Chey_ctrl.nc','lon',lon_msh);
    
Test_SW = ncread('Bias_S_N_CAM_POPgrid_coef_Chey_ctrl.nc','Bias_S_N_daily') ;
Test_lat = ncread('Bias_S_N_CAM_POPgrid_coef_Chey_ctrl.nc','lat') ;
Test_lon = ncread('Bias_S_N_CAM_POPgrid_coef_Chey_ctrl.nc','lon') ;
...
[Z_adjust(:,:,1),refvec_bias(:,:,1)] = geoloc2grid(squeeze(lat_T31(:,:,1)),squeeze(lon_T31(:,:,1)),squeeze(SST_T31(:,:,1)), cellsize);   
 ...
 lonlim = [1 360];
        ...
        fig_name = strcat('SW_Surf_N_bias_annual_fromFILE_CAM_POPgrid_diff_CERES');%,num2str(tt));
        fig_dum = figure(101);
      set(fig_dum, 'name', fig_name,'numbertitle','on');
      set(fig_dum,'units','inches','position',[0.3,0.3,12,9]);%,'PaperOrientation','landscape');
      set(fig_dum,'paperpositionmode','auto');
