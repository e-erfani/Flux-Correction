...
%%%%
cd (address)
    aa1=dir('OBS_pop_all_crrt_PreInd_T31_gx3v7.cam.h0.1181-1280._ANN_climo.nc');
    tt1=1; % ncdisp(aa(tt,1).name)
    filename1=aa1(tt1,1).name;
    latitude_C =ncread(filename1,'lat');
    longitude_C =ncread(filename1,'lon');
[latitude,longitude] = meshgrid(latitude_C,longitude_C);    
    gw=ncread(filename1,'gw'); 
      I=length(longitude);
      TAREA=repmat(gw,[1 I])';

    SW_S_N_mo_mean_intrp_CAMgrid = griddata(lon_fn_msh,lat_fn_msh,SH_S_average_OA,longitude,latitude,'natural');
    SW_S_N_mo_mean = SH_S_average_OA ;

nn = 0 ;
for tt1=1:length(aa1)
    nn = nn + 1 ;
    filename1=aa1(tt1,1).name;
    SW_N_all(:,:,nn) = ncread(filename1,'SHFLX'); % Sensible heat flux at surface (W/m2)
end
SW_N_mean = double(nanmean(SW_N_all,3));
TAREA2 = TAREA ;
...
cd /shared/SWFluxCorr/CESM/PreInd_chey_contr
    ...
    aa1=dir('chey_ctrl_rhminl_PreInd_T31_gx3v7.cam.h0.0471-0570._ANN_climo.nc');
    tt1=1; % ncdisp(aa(tt,1).name)
    filename1=aa1(tt1,1).name;
nn = 0 ;
for tt1=length(aa1)
    nn = nn + 1 ;
    filename1=aa1(tt1,1).name;
    SW_N_all_ctrl(:,:,nn) =  ncread(filename1,'SHFLX');
end
   SW_N_all_ctrl = double(SW_N_all_ctrl); 
TAREA2 = TAREA ;
...
[Z_adjust(:,:,1),refvec_bias(:,:,1)] = geoloc2grid(squeeze(lat_T31(:,:,1)),squeeze(lon_T31(:,:,1)),squeeze(SST_T31(:,:,1)), cellsize);   
 ...
 lonlim = [1 360];
        ...
        fig_name = strcat('SH_bias_CAM_OAFluxgrid_1181_1280');%,num2str(tt));
        fig_dum = figure(1);
...
[Z_adjust(:,:,1),refvec_bias(:,:,1)] = geoloc2grid(squeeze(lat_T31(:,:,1)),squeeze(lon_T31(:,:,1)),squeeze(SST_T31(:,:,1)), cellsize);   
 ...
 lonlim = [1 360];
        ...
        fig_name = strcat('SH_CAM_intrp_diff_1181_1280');%,num2str(tt));
        fig_dum = figure(2);
      set(fig_dum, 'name', fig_name,'numbertitle','on');
      set(fig_dum,'units','inches','position',[0.3,0.3,12,9]);%,'PaperOrientation','landscape');
      set(fig_dum,'paperpositionmode','auto');
...
AA=geoshow(squeeze(Z_adjust(:,:,1)), squeeze(refvec_bias(:,:,1)), 'DisplayType', 'texturemap','EdgeColor','flat');
  ...
  m_coast('linewidth',2,'color','black');
    Title_1 = strcat('Global Mean=',num2str(var_glb),',bias=',num2str(var_bias_glb),', RMSE=',num2str(var_rmse_glb),' Wm^-^2');   
    Title_1 = strcat('flx corr - ctrl, SH, CAM, Annual Mean=',num2str(var_glb_diff),' Wm^-^2');    
    title(Title_1,'fontsize',23,'fontweight','bold');
    cc = colorbar('peer',gca);
        colormap(brewermap(30,'*RdBu'))    
    set(gca,'Fontsize',20)
...
[Z_adjust(:,:,1),refvec_bias(:,:,1)] = geoloc2grid(squeeze(lat_T31(:,:,1)),squeeze(lon_T31(:,:,1)),squeeze(SST_T31(:,:,1)), cellsize);
 ...
 lonlim = [1 360];
        ...
        fig_name = strcat('SH_bias_CAM_OAFluxgrid_1181_1280_CTRL');%,num2str(tt));
        fig_dum = figure(3);
      set(fig_dum, 'name', fig_name,'numbertitle','on');
      set(fig_dum,'units','inches','position',[0.3,0.3,12,9]);%,'PaperOrientation','landscape');
      set(fig_dum,'paperpositionmode','auto');
