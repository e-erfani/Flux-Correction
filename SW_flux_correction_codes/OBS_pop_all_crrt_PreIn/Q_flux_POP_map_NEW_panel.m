...
%%%%%
 cd (address)
      fig_name = strcat('QFLUX_bias_POP_NEW_panel');
      fig_dum = figure(1);
...
axes(ha(1))
    ...
    [C, h] =  m_contourf(lon_c_m, lat_c_m, double(SW_mean_ctrl_interp_c - SW_mean_interp_obs_c),25);
    set(h,'LineColor','none')
    m_coast('linewidth',1,'color','black');
    Title_1  = 'ctrl-T31 - obs.';
    title(Title_1,'fontsize',15,'fontweight','bold');    
    colormap(brewermap(25,'*RdBu'))
    set(gca,'Fontsize',11)
...
axes(ha(2))
    m_proj('miller','long',[0 358],'lat',[-75 75])
    [C, h] =  m_contourf(lon_c_m2, lat_c_m2, double(SW_mean_ctrl_interp_c2 - SW_mean_interp_obs_c2),25);
    set(h,'LineColor','none')
    m_coast('linewidth',1,'color','black');
...
axes(ha(3))
    m_proj('miller','long',[0 358],'lat',[-75 75])
    [C, h] =  m_contourf(lon_c_m, lat_c_m, double(SW_mean_interp_c - SW_mean_interp_obs_c),25);
    set(h,'LineColor','none')
    m_coast('linewidth',1,'color','black');
    Title_1  = 'GC-T31 - obs.';
    title(Title_1,'fontsize',15,'fontweight','bold');    
...
axes(ha(4))
    m_proj('miller','long',[0 358],'lat',[-75 75])
    [C, h] =  m_contourf(lon_c_m2, lat_c_m2, double(SW_mean_interp_c2 - SW_mean_interp_obs_c2),25);
    set(h,'LineColor','none')
    m_coast('linewidth',1,'color','black');
    Title_1  = 'GC-f19 - obs.';
    title(Title_1,'fontsize',15,'fontweight','bold');
...
%%%%%%%%%%%%%%%%
      fig_name = strcat('QFLUX_diff_POP_panel');
        fig_dum = figure(2);
      set(fig_dum, 'name', fig_name,'numbertitle','on');
...
axes(ha(1))
    m_proj('miller','long',[0 358],'lat',[-75 75])
    [C, h] =  m_contourf(lon_c_m, lat_c_m, double(SW_mean_interp_c - SW_mean_ctrl_interp_c),20);
    set(h,'LineColor','none')
    m_coast('linewidth',1,'color','black');
...
axes(ha(2))
    m_proj('miller','long',[0 358],'lat',[-75 75])
    [C, h] =  m_contourf(lon_c_m2, lat_c_m2, double(SW_mean_interp_c2 - SW_mean_ctrl_interp_c2),20);
    set(h,'LineColor','none')
    m_coast('linewidth',1,'color','black');
