...
cd (address)
  ...
  var_mean = ncread(filename1,'SHF_QSW') ;
...
cd /shared/SWFluxCorr/CESM/PreInd_chey_contr
    aa1 = dir('tavg*.nc');
    filename1 = aa1(1,1).name;
    var_mean_ctrl = ncread(filename1,'SHF_QSW') ;
...
cd /shared/SWFluxCorr/high_res/OBS_pop_all_crrt_PreIn
  ...
  var_mean2 = ncread(filename1,'SHF_QSW') ;
...
cd /shared/SWFluxCorr/high_res/PreInd_f19_g16
    ...
    aa1 = dir('tavg*.nc');
    filename1 = aa1(1,1).name;
    var_mean_ctrl2 = ncread(filename1,'SHF_QSW') ;
...
idx = find(lat_pop_zon >= -40 & lat_pop_zon <= 40) ;
var_zonal2_intrp(idx) = SW_interp_zon(idx) ;

%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%
      fig_name = strcat('SW_zonal_NEW');
      fig_dum = figure(3);
...
h4 = plot(lat_pop_zon,var_zonal2_intrp,'b','linewidth',2) ;
    ...
    ylabel('Net SW (Wm^-^2)','fontsize',20,'fontweight','bold');
xlim([-74 74])
%ylim([-4.5 1.7])
...
hold on
%h4 = plot(lat_pop_zon2,var_zonal2 - SW_interp_zon2,'b','linewidth',2) ;
h4 = plot(lat_pop_zon,var_zonal2_intrp - SW_interp_zon,'b','linewidth',2) ;

    xlabel('Latitude (\circ)','fontsize',20,'fontweight','bold');
    ylabel('Net SW bias (Wm^-^2)','fontsize',20,'fontweight','bold');
xlim([-74 74])
ylim([-30 30])
...
%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%
      fig_name = strcat('SW_anomaly_zonal');
      fig_dum = figure(1);
      set(fig_dum, 'name', fig_name,'numbertitle','on');
      set(fig_dum,'units','inches','position',[0.5,0.5,9,8]);
      set(fig_dum,'paperpositionmode','auto');
...
hold on
    xlabel('Latitude (\circ)','fontsize',20,'fontweight','bold');
    ylabel('Net SW anomaly (Wm^-^2)','fontsize',20,'fontweight','bold');
xlim([-75 75])
%ylim([-2.5 3.1])
