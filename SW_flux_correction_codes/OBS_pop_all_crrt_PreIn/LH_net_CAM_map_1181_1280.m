...
[latitude,longitude] = meshgrid(latitude_C,longitude_C);    
    ...
    SW_S_N_mo_mean_intrp_CAMgrid = griddata(lon_fn_msh,lat_fn_msh,LH_S_average_OA,longitude,latitude,'natural');
...
for tt1=1:length(aa1)
    nn = nn + 1 ;
    filename1=aa1(tt1,1).name;
    SW_N_all(:,:,nn) = ncread(filename1,'LHFLX');
end
...
for tt1=length(aa1)
    nn = nn + 1 ;
    filename1=aa1(tt1,1).name;
    SW_N_all_ctrl(:,:,nn) =  ncread(filename1,'LHFLX');
end
...
[Z_adjust(:,:,1),refvec_bias(:,:,1)] = geoloc2grid(squeeze(lat_T31(:,:,1)),squeeze(lon_T31(:,:,1)),squeeze(SST_T31(:,:,1)), cellsize);   
 ...
 lonlim = [1 360];
        ...
        fig_name = strcat('LH_bias_CAM_OAFluxgrid_1181_1280');%,num2str(tt));
        fig_dum = figure(1);
...
[Z_adjust(:,:,1),refvec_bias(:,:,1)] = geoloc2grid(squeeze(lat_T31(:,:,1)),squeeze(lon_T31(:,:,1)),squeeze(SST_T31(:,:,1)), cellsize);   
 ...
 lonlim = [1 360];
        ...
        fig_name = strcat('LH_CAM_intrp_diff_1181_1280');%,num2str(tt));
        fig_dum = figure(2);
      set(fig_dum, 'name', fig_name,'numbertitle','on');
      set(fig_dum,'units','inches','position',[0.3,0.3,12,9]);%,'PaperOrientation','landscape');
      set(fig_dum,'paperpositionmode','auto');
...
AA=geoshow(squeeze(Z_adjust(:,:,1)), squeeze(refvec_bias(:,:,1)), 'DisplayType', 'texturemap','EdgeColor','flat');
    ...
    setm(gca,'FEdgeColor',[1 1 1]);
  m_coast('linewidth',2,'color','black');
    Title_1 = strcat('flx corr - ctrl, LH, CAM, Annual Mean=',num2str(var_glb_diff),' Wm^-^2');    
    title(Title_1,'fontsize',23,'fontweight','bold');
    cc = colorbar('peer',gca);
        colormap(brewermap(30,'*RdBu'))    
    set(gca,'Fontsize',20)
...
[Z_adjust(:,:,1),refvec_bias(:,:,1)] = geoloc2grid(squeeze(lat_T31(:,:,1)),squeeze(lon_T31(:,:,1)),squeeze(SST_T31(:,:,1)), cellsize);
 ...
 lonlim = [1 360];
        ...
        fig_name = strcat('LH_bias_CAM_OAFluxgrid_1181_1280_CTRL');%,num2str(tt));
        fig_dum = figure(3);
      set(fig_dum, 'name', fig_name,'numbertitle','on');
      set(fig_dum,'units','inches','position',[0.3,0.3,12,9]);%,'PaperOrientation','landscape');
      set(fig_dum,'paperpositionmode','auto');
